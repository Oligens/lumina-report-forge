...
export async function verifySource(source: ResearchSource): Promise<ResearchSource> {
  try {
    ...
    return { ...source, accessible: false, verifiedAt: now(), reason: "URL non vérifiable depuis le serveur." };
  }
}
...
export function evidenceForRule(rule: { id: string; statement: string; sourceIds: string[]; effectiveFrom: string }, sources: ResearchSource[]): EvidenceItem {
  const source = sources.find((candidate) => rule.sourceIds.includes(candidate.id) || rule.sourceId === candidate.id);
...
export async function buildResearchTrace(query: string, resolution: RegulatoryResolution): Promise<ResearchTrace> {
  const registrySources = regulatorySources(resolution);
  const webSources = await liveWebResearch(query);
  const merged = new Map<string, ResearchSource>();
  for (const source of [...registrySources, ...webSources]) merged.set(source.url, source);
  const verifiedSources = await Promise.all([...merged.values()].slice(0, 20).map(verifySource));
